...
Modified by Xuehong Zhang March 03, 2016
...
class fastQRead:
    def __init__(self, in1, in2, in3, in4):
        ...
        self.name=in1.strip().strip("@").split(' ')[0]
        self.seq=in2.strip()
        self.spacer="+"
        self.qual=in4.strip()
...
def tagExtractFxn(x):
    ...
    '''
    tagindex1 = x[0].find("TGACT", 6, 14)
    tagindex2 = x[1].find("TGACT", 6, 14)
    tagindex = 6
    if (tagindex1 >0 and tagindex2 > 0):
        tag1 = x[0][:tagindex1]
        tag2 = x[1][:tagindex2]
    else:
        tag1 = x[0][:tagindex]
    ...
    return (tag1, tag2, tagindex1, tagindex2)
...
def hdrRenameFxn(x, y, z):
    ...
    *read designation from original header*
    ...
    return("|%s_%s/%s" % (y, z,  x[0:]))
#     return("%s|%s%s/%s" % (x[0:3], y, z,  x[4:]))
...
def main():
    ...
    while isEOF==False:
        ...
        if read1 == "EOF" or read2 == "EOF":
            ...
            #extract tags
            tag1, tag2, tagindex1, tagindex2 = tagExtractFxn((read1.seq, read2.seq))
            if (tagindex1 <0 or tagindex2 < 0):
            	nospacer += 1
            else:    
            ...
            #fastq reconstruction
                if (tag1.isalpha() and tag1.count('N') == 0) and (tag2.isalpha() and tag2.count('N') == 0):
                    rOut1 = read1[len(tag1) + o.slength:]
                    rOut2 = read2[len(tag2) + o.slength:]
                    out1.write(rOut1)
                    out2.write(rOut2)
                    goodreads += 1
                else: 
